...
def generate(args,tokenizer,model,train_dataloader):
	...
	print('passing rate: ',scores.sum().item()/scores.shape[0])
	#print(querys.shape)
	...
	for batch in tqdm(dataloader,desc=description):
		...
		for generated_ids4problem in generated_ids:
			...
			for generated_str in generated_strs:
				...
				if did_compile:
					onegood = True
			scores.append(onegood)
		if description == "Validating" and count == 49: break
...
if __name__ == "__main__":
	...
	train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=generate_args.batch_size)
	test_dataloader = DataLoader(test_dataset, sampler=test_sampler, batch_size=generate_args.batch_size)
	vali_dataloader = DataLoader(vali_dataset, sampler=vali_sampler, batch_size=generate_args.batch_size)

	#Testorvali(generate_args,tokenizer,model,test_dataloader,"Testing")

	for rsteps in range(training_args.RL_steps):
		RLsteps = rsteps
		output_dataset = generate(generate_args,tokenizer,model,train_dataloader)
		...
		trainer.train()
		trainer.save_state()
		#wandb.log({"epoch": epoch, "loss": loss}, step=rsteps)
		safe_save_model_for_hf_trainer(trainer=trainer, output_dir=training_args.output_dir)
		if rsteps%4==0:
			Pass_rate = Testorvali(generate_args,tokenizer,model,vali_dataloader,"Validating")
			if data_args.report:
				wandb.log({
					'Pass rate': Pass_rate,
					'rl_step': RLsteps  # Logging the RL step can be helpful
